...
coherence-cli vendors its `.claude/skills/` from **guildmaster** — the
...
Every vendored `SKILL.md` carries `type: command`. coherence-cli
...
#   - consumer-identifying prose: `guildmaster` → `coherence-cli` (NOT
...
If a re-sync would lose a coherence-cli adaptation, lift the change
...
#     → `which coherence-cli vendors from guildmaster`
